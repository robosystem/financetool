...
st.write("Qualsiasi investimento comporta un grado di rischio, inclusa la possibile \
...
if target > 0 and volume > 0:
	...
	incremento = ((qta*target)-(qta*acquisto))/volume
	profitto = round((qta*target)-volume,2)
	st.write("Guadagno atteso", f'{incremento:.2%}'+" ("+str(profitto)+")")
stop = st.number_input("Stop Loss", min_value=0.0)
if stop > 0 and volume > 0:
	st.write("Controvalore minimo", round(qta*stop, 2))
	decremento = ((qta*stop)-(qta*acquisto))/volume
	perdita = round((qta*stop)-volume,2)
	st.write("Perdita attesa", f'{decremento:.2%}'+" ("+str(perdita)+")")
...
if cont:
	...
	if add > 0 and qta > 0:
		...
		if acquisto > 0 and prezzo > 0:
			...
			#-195.96 (-21.49%)
			hyp_incremento = (target*(qta+add)-(volume+costo))/(volume+costo)
			hyp_profitto = round((target*(qta+add)-(volume+costo)),2)
			hyp_decremento = (stop*(qta+add)-(volume+costo))/(volume+costo)
			hyp_perdita = round((stop*(qta+add)-(volume+costo)),2)
			delta_incremento = hyp_incremento-incremento
			delta_decremento = decremento-hyp_decremento
			col1, col2, col3 = st.columns(3)
			with col1:
				st.write("#### Strategia")
				st.write("Profitto", f'{incremento:.2%}'+" ("+str(profitto)+")")
				st.write("Perdita ", f'{decremento:.2%}'+" ("+str(perdita)+")")
			with col2:
				st.write("#### Rimodulazione")
				st.write("Profitto", f'{hyp_incremento:.2%}'+" ("+str(hyp_profitto)+")")
				st.write("Perdita ", f'{hyp_decremento:.2%}'+" ("+str(hyp_perdita)+")")
			with col3:
				st.write("#### Δ")
				st.write("Profitto", f'{(delta_incremento):.2%}')
				st.write("Perdita ", f'{(delta_decremento):.2%}')
